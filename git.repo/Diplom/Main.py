...
class MainVkApi:
    ...
    def getphotolist_vk(self, iduser, idalbum='profile'):
        ...
        for each_photo in self.resp_photo.json()['response']['items']:
            photo_list.append({'lt_size': each_photo['sizes'][-1]['height'] * each_photo['sizes'][-1]['width'],
                               'size_type': each_photo['sizes'][-1]['type'],
                               'link_max_photo': each_photo['sizes'][-1]['url'], 'likes': each_photo['likes']['count']})
            # photo_list[each_photo['id']] = {'liles': each_photo['likes']['count']}

        print(photo_list)
        photo_list = sorted(photo_list, key=lambda k: k['lt_size'], reverse=True)
        print(photo_list)
        print(self.resp_photo.json())
